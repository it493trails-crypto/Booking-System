...
function sendPaymentLinkToCustomer(rentalAmount, description, email) {
  ...
  const paymentLink = createPaymentLinkAndEmail(stripe, booking);
 ...
 * @returns {Object} Stripe Payment Link object
 */
function createPaymentLinkAndEmail(stripe, booking) {

  // === RENTAL PRODUCT ===
  const rentalProduct = createProduct(
    stripe.secretKey,
    booking.description
  );

  const rentalPrice = createPriceFromInteger(
    stripe.secretKey,
    rentalProduct.id,
  ...
  const depositProduct = createProduct(
    stripe.secretKey,
    booking.description + ' - Security Deposit'
  );

  const depositPrice = createPriceFromInteger(
    stripe.secretKey,
    depositProduct.id,
  ...
  const paymentLink = createPaymentLinkFromPrices(
    stripe.secretKey,
    rentalPrice.id,
    depositPrice.id
  );
 ...
 */
function createPaymentLinkFromPrices(secretKey, rentalPriceId, depositPriceId) {
